"""Implementation of a space consisting of finitely many elements from given set."""
...
class Set(Space[int]):
    r"""A space consisting of finitely many elements from given set.

    This class represents a finite subset of integers, more specifically a set from given set.
    ...
    def __init__(
        ...
        r"""Constructor of :class:`Set` space.

        Args:
            vec: set of elements to choose from
    ...
    def sample(self, mask: Optional[np.ndarray] = None) -> int:
        ...
        if mask is not None:
            ...
            if np.any(valid_action_mask):
                return int(
                    self.vec[self.np_random.choice(np.where(valid_action_mask)[0])]
                )
            else:
                return self.vec[0]

        return self.vec[self.np_random.integers(len(self.vec))]
    ...
    def contains(self, x) -> bool:
        ...
        if isinstance(x, int):
            ...
            return False
        return x in self.vec

    def __repr__(self) -> str:
        """Gives a string representation of this space."""
        return "Set({%s})".format(self.vec) 
